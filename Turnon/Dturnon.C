...
void Dturnon(TString inputname, std::vector<TString> HLTcut, std::vector<TString> L1cut)
{
  ...
  for(int k=0;k<HLTcut.size();k++)
    {
      hnorm[k] = new TH1D(Form("hnorm_%d",k),"",50,0,80);
      hnorm[k]->Sumw2();
      hdenorm[k] = new TH1D(Form("hdenorm_%d",k),"",50,0,80);
      hdenorm[k]->Sumw2();
  ...
  for(int i=0;i<eventnumber;i++)
    {
      ...
      for(int j=0;j<Dsize;j++)
        {
          if((abs(Dmass[j]-1.8696)<0.1)&&(DsvpvDistance[j]/DsvpvDisErr[j])>2.5&&Dchi2cl[j]>0.10&&Dtrk1highPurity[j]&&Dtrk1Pt[j]>1&&abs(Dtrk1Eta[j])<2.0&&Dtrk2highPurity[j]&&Dtrk2Pt[j]>2&&abs(Dtrk2Eta[j])<2.0&&TMath::Cos(Ddtheta[j])>0.9&&Dalpha[j]<0.12 &&
...
int main()
{
  ...
  //         std::vector<TString>{"L1_SingleJet16_Final", "L1_SingleJet24_Final", "L1_SingleJet40_Final", "L1_SingleJet44_Final"});
  Dturnon("/export/d00/scratch/jwang/HLTppRef/crab_HLT_V14_pp_20171023_prompt_D0_5020GeV_pythia8_20171005_nocuts_pthatweight.root",
